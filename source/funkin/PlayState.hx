...
class PlayState extends MusicBeatState
{
	...
	private final singLeftKey = Registry.keybindings.get(new Identifier("basegame", "left"));
	private final singDownKey = Registry.keybindings.get(new Identifier("basegame", "down"));
	private final singUpKey = Registry.keybindings.get(new Identifier("basegame", "up"));
	private final singRightKey = Registry.keybindings.get(new Identifier("basegame", "right"));

	private final pauseKey = Registry.keybindings.get(new Identifier("core", "pause"));
	...
	override public function create()
	{
		...
		trace('Mod chart: ' + executeModchart + " - " + Paths.lua(SONG.id.path + "/modchart"));
		...
		if (SONG.id.namespace == "basegame") switch (SONG.id.path)
		{
			...
			case 'thorns':
				dialogue = [
					...
					":dad:I'll make her father pay for what he's done to me and all the others,,,,", // why are these commas lmao
					":dad:I'll beat you and make you take my place.",
					":dad:You don't mind your bodies being borrowed right? It's only fair..."
				];
		}

		stage = new Stage(SONG.info.stage).load(true);
		...
		if (FlxG.save.data.songPosition) // I dont wanna talk about this code :(
			{
				...
				var songName = new FlxText(songPosBG.x + (songPosBG.width / 2) - (SONG.meta.name.length * 5),songPosBG.y,0,SONG.meta.name, 16);
				if (PlayStateChangeables.useDownscroll)
					songName.y -= 3;
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.meta.name + " - " + CoolUtil.difficultyFromInt(storyDifficulty) + (Main.watermarks ? " | KE " + MainMenuState.kadeEngineVer : ""), 16);
		...
		var iconP1Id = Registry.characters.get(SONG.info.player).icon;
		iconP1 = new HealthIcon(iconP1Id, true);
		iconP1.y = healthBar.y - (iconP1.height / 2);
		add(iconP1);

		var iconP2Id = Registry.characters.get(SONG.info.opponent).icon;
		...
		if (isStoryMode)
		{
			if (curSong.namespace == "basegame") switch (curSong.path)
			{
	...
	private function handleInput(evt:KeyboardEvent):Void { // this actually handles press inputs
		...
		// Convert the key code from OpenFL to HaxeFlixel. This may not actually
		// be necessary because I think they're the same (see FlxKeyboard.hx
		// lines 32-40), but I'm doing it just in case.
		@:privateAccess
		var pressedInput = FlxG.keys.getKey(evt.keyCode);
		var pressedKey = pressedInput != null ? pressedInput.ID : FlxKey.NONE;

		var keybindings:Array<Keybinding> = [singLeftKey, singDownKey, singUpKey, singRightKey];

		var trackIndex: Int = -1;
		for (index => keybinding in keybindings)
		{
		...
		{
			trace(String.fromCharCode(evt.charCode) + " " + pressedKey);
		}

		if (trackIndex == -1)
			return;

		var ana = new Ana(Conductor.songPosition, null, false, "miss", trackIndex);

		var dataNotes = [];
		notes.forEachAlive(function(daNote:Note)
		{
			if (daNote.canBeHit && daNote.mustPress && !daNote.tooLate && !daNote.wasGoodHit && daNote.noteData == trackIndex)
	...
	function startSong():Void
	{
		...
		if (!paused)
		{
			FlxG.sound.playMusic(SONG.id.getAssetPath("songs", "instrumental", Paths.SOUND_EXT), 1, false);
		}
	...
	private function generateSong():Void
	{
		...
		// NEW SHIT
		noteData = songData.chart.sections;
		...
		for (section in noteData)
		{
			for (songNotes in section.notes)
			{
				...
				if (daStrumTime < 0)
					daStrumTime = 0;
				var daNoteData:Int = Std.int(songNotes.column % 4);

				var gottaHitNote:Bool = songNotes.column < 4;
				...
				var swagNote:Note = new Note(daStrumTime, daNoteData, songNotes.type, oldNote);
				...
				for (susNote in 0...Math.floor(susLength))
				{
					...
					var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + Conductor.stepCrochet, daNoteData, songNotes.type, oldNote, true);
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			var noteTypeCheck:Identifier = SONG.info.uiStyle;

			if (PlayStateChangeables.Optimize && player == 0)
				continue;

			switch ([noteTypeCheck.namespace, noteTypeCheck.path])
			{
				case ["basegame", "pixel"]:
	...
	override public function update(elapsed:Float)
	{
		...
		if (pauseKey.justPressed && startedCountdown && canPause)
		{
		...
		if (generatedMusic && PlayState.SONG.chart.sections[Std.int(curStep / 16)] != null)
		{
			...
			var focusedCharacter = switch (SONG.chart.sections[Std.int(curStep / 16)].cameraFocus)
			{
				case Player: stage.player;
				case Opponent: stage.opponent;
				case Girlfriend: stage.girlfriend;
			};
			var cameraPosition = focusedCharacter.getCameraPosition();
			camFollow.setPosition(cameraPosition.x, cameraPosition.y);
			cameraPosition.put();

			// // TODO: Handle these offsets.
			// if (stage.id.namespace == "basegame") switch (stage.id.path)
			// {
			// 	case 'limo':
		...
		if (generatedMusic)
			{
				notes.forEachAlive(function(daNote:Note)
				{
					...
					if (!daNote.modifiedByLua)
						{
							...
							{
								if (daNote.mustPress)
									daNote.y = (playerStrums.members[Math.floor(Math.abs(daNote.noteData))].y + 0.45 * (Conductor.songPosition - daNote.strumTime) * FlxMath.roundDecimal(PlayStateChangeables.scrollSpeed == 1 ? SONG.chart.scrollSpeed : PlayStateChangeables.scrollSpeed, 2));
								else
									daNote.y = (strumLineNotes.members[Math.floor(Math.abs(daNote.noteData))].y + 0.45 * (Conductor.songPosition - daNote.strumTime) * FlxMath.roundDecimal(PlayStateChangeables.scrollSpeed == 1 ? SONG.chart.scrollSpeed : PlayStateChangeables.scrollSpeed, 2));
					...
					{
						if (SONG.id != new Identifier("basegame", "tutorial"))
							camZooming = true;

						var noteTypeMetadata = Registry.noteTypes.get(daNote.noteType);
						var altAnim:String = noteTypeMetadata.animationSuffix != null ? "-" + noteTypeMetadata.animationSuffix : "";
		...
		private function keyShit():Void // I've invested in emma stocks
			{
				...
				var holdArray:Array<Bool> = [
					...
					singDownKey.pressed,
					singUpKey.pressed,
					singRightKey.pressed
				];
				var pressArray:Array<Bool> = [
					singLeftKey.justPressed,
					singDownKey.justPressed,
					singUpKey.justPressed,
					singRightKey.justPressed
				];
				var releaseArray:Array<Bool> = [
					singLeftKey.justReleased,
					singDownKey.justReleased,
					singUpKey.justReleased,
					singRightKey.justReleased
				];
				// #if windows
				// if (luaModchart != null){
